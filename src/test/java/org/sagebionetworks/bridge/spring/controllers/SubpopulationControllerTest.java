...
public class SubpopulationControllerTest extends Mockito {
    ...
    public void createSubpopulation() throws Exception {
        String json = TestUtils.createJson(
                "{'guid':'junk','name':'Name','defaultGroup':true,'description':'Description','required':true,'criteria':{'minAppVersion':2,'maxAppVersion':10,'allOfGroups':['requiredGroup'],'noneOfGroups':['prohibitedGroup']}}");
    ...
    public void updateSubpopulation() throws Exception {
        String json = TestUtils.createJson(
                "{'name':'Name','description':'Description','defaultGroup':true,'required':true,'criteria':{'minAppVersion':2,'maxAppVersion':10,'allOfGroups':['requiredGroup'],'noneOfGroups':['prohibitedGroup']}}");
        mockRequestBody(mockRequest, json);
